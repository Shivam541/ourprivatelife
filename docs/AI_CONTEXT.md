...
1. The setup page requests camera and microphone access; the user grants browser permission.
2. Select **Create invite** from the connection choices. The app reveals the caller panel, creates a peer connection, waits for ICE gathering, stores the Base64 invite in the current browser session, and reveals **Copy invite code**.
3. Send the invite code to the receiver through an out-of-band channel.
4. Select **Paste response** (or paste manually) into the caller panel and select **Start call**.
5. The dedicated call view opens. Keep the tab open until connected or disconnected.

### Receiver

1. Select **Join a session** from the connection choices, then select **Paste invite** (or paste the received invite code manually) into the visible input.
2. Select **Accept invite & create response**. The app produces a response and hides the invite input.
...
Only the relevant text input should be visible at a time. Do not reintroduce exposed JSON textareas as output; codes are copied with buttons and can be pasted from the clipboard with a user-initiated control (manual paste remains the fallback if clipboard permission is unavailable). The call view contains the feeds plus mic, camera, quality, layout, switch-screen, full-screen, chat, and leave controls. The default camera profile is 720p/18fps. Quality offers independent 480p/720p/1080p/1440p resolution and 18/24/30fps selection. Applying a profile acquires a replacement camera track, calls `replaceTrack()` on the active camera sender, sets compatible sender `maxBitrate`, `maxFramerate`, and `degradationPreference`, then stops the old track; it must not require a new manual code exchange or end the call. Its Audio quality section shows the active microphone's browser-reported sample rate, channel count, bit depth, processing state, and outgoing bitrate target. It does not claim to measure real-time network bitrate. Audio prioritizes fidelity over call cleanup: request 48 kHz, 24-bit stereo audio, disable echo cancellation/noise suppression/automatic gain, mark the track `music`, and request a 510 kbps audio-sender cap. These are preferences only; device, browser, network, codec negotiation, and playback hardware may impose lower quality. Recommend headphones because echo cancellation remains intentionally off. If a microphone track ends or becomes muted by an iPad/iPhone/macOS browser audio-session interruption, show a blue **Reconnect microphone** control. It must reacquire that same high-fidelity audio profile after the user taps it, replace the existing WebRTC sender track, stop/remove stale local audio tracks, and remain a retryable explicit user action if device access fails. Chat must remain a transient data-channel feature: enable it only when the channel is open, close only the panel when the user toggles it, and clear messages and object URLs when the call ends. An incoming text message, completed file transfer, or shared-timer start/reset must play a short generated local pop after a user gesture has enabled browser audio. If Chat is closed, it must also show a blue unread dot on its control, set an accessible “new messages” label, and clear the indicator when Chat opens or history clears. Typing `/` opens a filtered command menu; it must support click, ↑/↓ + Enter selection, Escape dismissal, and accessible listbox semantics. `/clear` sends a dedicated data-channel event that clears both active chat views, but cannot revoke a previously downloaded file. `/timer minutes` sends one dedicated data-channel event that starts a shared whole-minute countdown (1–180); each browser performs its own ticking, and `/timer reset` restarts the active timer for both views. Timers are transient and are cleared by `/clear` or call disconnect. Keep `/download-all`, `/files`, `/status`, `/close`, `/mas`, `/gpt`, `/gem`, `/gs`, `/ig`, `/tg`, `/wa`, `/search`, and `/help` local-only utility commands: they must never run from received messages. `/mas`, `/gpt`, `/gem`, `/gs`, `/ig`, `/tg`, `/wa`, and `/search query` open a user-initiated new browser tab with `noopener,noreferrer`; Telegram and WhatsApp may hand off to an installed app, but the static app cannot require this or select Chrome over another default browser. Only `http://` and `https://` URLs become links, using `noopener noreferrer` and a user click to open a new tab. Treat common image MIME types and image file extensions as inline previews. A preview load failure (such as an unsupported HEIC codec) must retain the download and show a clear browser-compatibility fallback. Videos, PDFs, and other file types appear as full-name download cards, preserving the original uploaded bytes. On phones, the chat panel fills the screen and its close control returns to video. Recording is deliberately unavailable. The full-screen control uses the browser Fullscreen API to hide browser chrome; it must reflect an Esc/system exit and be disabled when unavailable. The layout control switches between picture-in-picture and side-by-side feeds in landscape. Portrait always shows two equal stacked panels; neither tile can float or be dragged. Call-control labels are visually hidden at every size, but their accessible labels and tooltips must remain available.

### Self-destruct command

`/self-destruct minutes` accepts whole minutes from 1 through 180. It is separate from the chat-only `/timer`: it sends a dedicated data-channel event, shows a destructive countdown in each chat view, and calls the existing `disconnect()` cleanup on both devices at expiry. Starting it again replaces its current countdown. It remains transient and is cleared by call disconnect.

### Screen sharing

After the peer connection and its `chat` data channel are open, `getDisplayMedia()` may add a screen video track and, only when the browser supplies it, a computer/tab audio track. Screen capture targets up to 1280×720 at 15 fps, with the matching sender capped at 1.2 Mbps / 15 fps to reduce battery, heat, and outgoing bandwidth; these are browser preferences rather than guarantees. Set the screen track's `contentHint` to `detail` so documents and text remain prioritized over motion. Screen media is one-way: one active sender and one receiver. When `screen-share-state` says the friend is sharing, disable the local Share screen control until that state ends; do not permit simultaneous screen streams. The call UI shows the screen as the main stage and both cameras in a draggable floating tile. The data channel carries internal `webrtc-offer` and `webrtc-answer` messages for automatic renegotiation; users must never exchange another manual code. Use the existing caller/receiver role as the polite/impolite perfect-negotiation roles so simultaneous renegotiation can recover from an offer collision. Removing the screen tracks must renegotiate again and stop capture tracks. Browser/OS choice determines whether computer audio is available, so do not promise it.
...
- Camera capture defaults to the user-selected 720p/18fps profile and uses `ideal` camera constraints, allowing the browser to select the closest supported mode. WebRTC can still adapt transmitted quality for device performance or network conditions.
